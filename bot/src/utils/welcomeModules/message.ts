...
export async function formatMessage(
  ...
  try {
    ...
    if (!messageValidated.success) {
      console.error(
        `Message validation failed: ${messageValidated.error.errors
          .map((e) => e.message)
          .join(", ")}`
      );
      // Return a safe default message instead of throwing
      return {
        content: "Welcome!",
        embeds: [],
        files: [],
    ...
    if (message.embeds) {
      message.embeds.forEach((embed, index) => {
        ...
        if (!embedValidated.success) {
          console.error(
            `Embed ${index} validation failed: ${embedValidated.error.errors
              .map((e) => e.message)
              .join(", ")}`
          );
          // Remove invalid embed instead of crashing
          message.embeds = message.embeds?.filter((_, i) => i !== index);
        }
      });
    }
    ...
    console.error("Critical error in formatMessage:", err);
